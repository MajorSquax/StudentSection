...
function Marketplace() {
  ...
  useEffect(() => {
    ...
    axios.get('https://studentsection.xyz/flaskapi/tickets')
  ...
  const handlePurchase = async (ticketId, sellerId) => {
    setMessage('');
    try {
      const buyerRes = await axios.get('https://studentsection.xyz/flaskapi/users/me', { withCredentials: true });
      const buyerData = buyerRes.data;
      if (!buyerData || !buyerData.email) {
        throw new Error('Please log in to purchase tickets.');
      }
      const sellerRes = await axios.get(`https://studentsection.xyz/flaskapi/users/${sellerId}/profile`, { withCredentials: true });
      // (Optional: we could use sellerRes.data.profile.email if needed)
      
      await axios.post(`https://studentsection.xyz/flaskapi/tickets/${ticketId}/purchase`, {}, { withCredentials: true });
