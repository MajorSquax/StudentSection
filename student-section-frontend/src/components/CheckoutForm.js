...
const CheckoutForm = () => {
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = (e) => {
      if (!purchaseFinalized.current) {
        navigator.sendBeacon(`https://studentsection.xyz/flaskapi/tickets/${ticketId}/purchase`);
      }
    };
  ...
  useEffect(() => {
    if (timeLeft === 0 && !purchaseFinalized.current) {
      ...
      fetch(`https://studentsection.xyz/flaskapi/tickets/${ticketId}/purchase`, {
        method: 'DELETE',
        credentials: 'include'
      })
        .then(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`https://studentsection.xyz/flaskapi/tickets/${ticketId}/purchase/intent`, {
        method: 'POST',
        credentials: 'include'
      });
      ...
      if (result.error) {
        ...
        if (pi.status === 'succeeded') {
          ...
          const confirmRes = await fetch(`https://studentsection.xyz/flaskapi/tickets/${ticketId}/purchase/confirm`, {
            method: 'POST',
            credentials: 'include'
          });
